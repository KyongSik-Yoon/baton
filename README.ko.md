...
**Claude Code 세션을 순수 오케스트레이터로** 돌린다 — 바톤을 든 사람은 소리를 내지 않는다: 사고·분해·위임·리뷰만 하고 직접 수정은 절대 하지 않는다. 부모 모델은 전제가 아니라 프로파일이다: **Opus 5 부모**면 판단이 필요한 곳에만 Fable 5를 불러오는 저비용 오케스트레이션, **Fable 5 부모**면 최상위 판단력이 직접 지휘하되 구현은 여전히 저렴한 핀 고정 워커가 맡는 구성이 된다. 강제 계층은 부모 모델을 아예 들여다보지 않으므로 두 프로파일 모두 동일하게 강제된다.

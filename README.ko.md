...
**세션을 순수 오케스트레이터로** 돌린다 — 사고·분해·위임·리뷰만 하고 직접 수정은 절대 하지 않는다. 이름과 달리 부모 모델은 전제가 아니라 프로파일이다: **Opus 5 부모**면 판단이 필요한 곳에만 Fable 5를 불러오는 저비용 오케스트레이션, **Fable 5 부모**면 최상위 판단력이 직접 지휘하되 구현은 여전히 저렴한 핀 고정 워커가 맡는 구성이 된다. 강제 계층은 부모 모델을 아예 들여다보지 않으므로 두 프로파일 모두 동일하게 강제된다.
...
- **`parent=opus`** (기본): 트리거에서 Fable에 자문하되, 질문마다 새로 띄우지 않고 **하나의 지속 advisor 에이전트**를 SendMessage로 이어 쓴다. `advisor=none`으로 두면 — 예컨대 Fable 쿼터를 다 쓴 뒤 — 설계·모호성 트리거는 AskUserQuestion으로 사용자에게 가고, 최종 리뷰 트리거는 `opus-5-router:reviewer-xhigh`로 간다.
- **`parent=fable`**: 오케스트레이터 자신이 최상위 판단 티어다 — 설계·모호성 트리거는 그 자리에서 직접 결정하고, 최종 리뷰 트리거만 외부로 보낸다(`opus-5-router:reviewer-xhigh` — 능력이 아니라 독립성을 위해). 이 프로파일에서 advisor 기본값은 `none`이고, `advisor=fable`은 같은 티어의 fresh-context 2차 의견을 opt-in하는 것이다.

`/opus5-router on`에 `parent=`를 명시하지 않으면 스킬이 세션 자신의 모델 정체성으로 프로파일을 기본 설정한다.

## 사용법

```
/opus5-router on               # parent 자동 감지; advisor는 opus면 fable, fable이면 none
/opus5-router on advisor=none  # Fable 없는 opus 부모 모드
/opus5-router on parent=fable  # Fable 부모 프로파일 명시
/opus5-router advisor none     # 모드 유지한 채 advisor만 전환
/opus5-router parent fable     # 모드 유지한 채 부모 프로파일만 전환
/opus5-router status
/opus5-router off
```
...
- fable-router 플러그인의 auto 모드와 동시에 켜지 말 것 — `parent=fable`이라도 같은 세션 위에서 경합하는 두 라우팅 체계다. `/opus5-router on`은 두 플래그가 겹치면 경고한다.

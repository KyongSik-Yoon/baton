...
토큰 절약 모드 두 가지를 담은 Claude Code 플러그인: **fable-router** 스킬(Fable 5가 부모로서 위임 가능한 단계를 Opus/Sonnet/Haiku에 내려보냄 — 아래에 먼저 설명)과 **opus-orchestrator** 모드(Opus 5가 훅으로 강제되는 순수 오케스트레이터가 되고 Fable은 advisor로만 자문). [gpt-5.6-router](https://github.com/volition79/gpt-5.6-router)에서 영감을 받아(inspired by) Claude Code용으로 이식했다.

fable-router 스킬은 Fable 5(부모)의 토큰을 아끼기 위해 위임 가능한 작업 단계를 Agent 도구의 `model` 오버라이드로 Opus/Sonnet/Haiku에 배정한다.
...
참고: 수동 설치 시 워커는 `opus-5-router:` 접두사 없이 `worker-low` 등으로 등록된다. 플러그인 설치가 문서화된 기본 경로.
...
- 스카우트가 Haiku를 유지하는 이유: 정찰 비용은 input 토큰이 지배하는데 effort 설정은 input 비용을 줄여주지 못하고, Sonnet 5의 신형 토크나이저(같은 텍스트가 ~30% 더 많은 토큰)까지 겹치면 스티커 3배 가격 차가 실질 ~4배로 벌어진다(2026-08-31 종료되는 인트로 가격 기준으로도 ~2.6배). 스캔이 아니라 해석이 필요한 정찰은 Haiku의 floor를 넘으므로 `opus-5-router:worker-low` + `model: sonnet`으로 보낼 것.

...
2. **Recon** via `fable-router:scout` (pinned Haiku) — never via built-in Explore, which inherits the expensive session model. Scouts return facts and paths, not dumps.
